...
% Criteria for regularizing the concentration EVP 
%
...
%         mnIndx = min( nChannels, length( roiColumns ) );
        ...
        total = sum(concEigenvalues);
        rsum = 0; mStar = 0;
        while rsum < conc.threshold
            mStar = mStar +1;
            rsum = rsum + (concEigenvalues( mStar )/total);
        end
        fprintf( 1, '   @ ROI on voxel = %g, mStar = %g\n', vx, mStar);
