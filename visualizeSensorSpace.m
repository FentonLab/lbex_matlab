...
error('Need to implement channel grouping component');
...
if baseSubtract
    ...
    for i = 1:nTrials
        ...
        for c = 1:size(power{i},1)
            avgS(:, c) = mean( squeeze(power{i}(c,indf,:)), 2); % frequency x windows/time
        end
        avgmax = max( avgmax, max(avgS(:)) );
        avgmin = min( avgmin, min(avgS(:)) );
    end
end
...
if baseSubtract
    clims =[log10(pmin) - log10(avgmin), log10(pmax) - log10(avgmax)];
end
